...
namespace Api.Services
{
    public class ApiWorkerService : BackgroundService
    {
        ...
        private readonly System.Int32 KafkaConsumerTimeoutPeriodInMilliSeconds = 30000;
        public ApiWorkerService(IServiceProvider serviceProvider, ILogger<ApiWorkerService> logger, IConfiguration config, IEmailNotificationService emailNotification)
        {
        ...
        public async Task KafkaConsumerPoll(CancellationToken stoppingToken)
        {
            ...
            try
            {
                ...
                if (response != null)
                {
                    ...
                    // Code for Lead Created Email Notification through Kafka

                    //// Convert message in JsonObject
                    //JObject jsonObject = JObject.Parse(consumeResultString);

                    //// Extract the value of the "Event" property
                    //string eventType = jsonObject["AuditableSourceEventName"].Value<string>();

                    //if (eventType == "LeadCreatedEvent")
                    //{
                    // var createNotification = JsonSerializer.Deserialize<EmailNotification>(consumeResultString);
                    //    await _emailNotification.SendEmailFromTemplate("Lead Created");
