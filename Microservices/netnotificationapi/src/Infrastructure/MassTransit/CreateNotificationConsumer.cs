...
public class CreateNotificationConsumer : IConsumer<CreateNotificationMessage>
{
    ...
    public CreateNotificationConsumer(ILogger<CreateNotificationBatchConsumer> logger, IConfiguration config, IEndpoint endpoint, IDapper dapper, IEmailNotificationService emailNotificationService)
    {
        _logger = logger;
        _config = config;
        _endpoint = endpoint;
    ...
    public async Task Consume(ConsumeContext<CreateNotificationMessage> context)
    {
        ...
        ConsumeContext<CreateNotificationMessage> item = context;
        ...
        try
        {
            if (msg.Type == "ZeptoMail")
            {
                ...
                if (notification == null)
                {
                    _logger.LogInformation(
                        $"CreateNotificationConsumer.Consume - No data after deserialization for MessageId={item.MessageId} | Type={msg.Type}");
            ...
            else if (msg.Type == "MSGraph")
            {
                ...
                //To generate an error for testing retry mechanism and observer code.
