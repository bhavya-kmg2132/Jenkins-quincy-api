...
namespace Application.IntegrationTests
{
    public class EndpointTestBase
    {
        ...
        public void BaseSetUp()
        {
            ...
            {
                // Use the in-memory WebApplicationFactory server; base URL comes from its HttpClient
                ServerUrl = "";
            }

            //Initiate objects for derived classes
            Request = new RestRequest();
            if (EndPointsSettings.Test_Environment == "local")
            {
                Client = new RestClient(Testing.GetTestHttpClient());
            }
            else
            {
        ...
        private void handle_authentication()
        {
            ...
            if (Testing.RequestUid.Contains("Chris.Green"))
            {
                TokenRequest = new RestRequest(EndPointsSettings.ApiEndPoint.GetTokenServerUrl + EndPointsSettings.ApiEndPoint.GetTokenForUserByParamKMG, Method.Get);
            }
            else if (Testing.RequestUid.Contains("John.Smith"))
            {
            ...
            else if (Testing.RequestUid.Contains("Level1User1"))
            {
            ...
            else if (Testing.RequestUid.Contains("Level2User1"))
            {
            ...
            else if (Testing.RequestUid.Contains("Level3User1"))
            {
            ...
            Auth_Token = "Bearer " + JsonSerializer.Deserialize<string>(postJsonResult);
