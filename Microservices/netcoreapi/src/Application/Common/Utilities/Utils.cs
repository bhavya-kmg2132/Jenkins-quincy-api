...
namespace Application.Common.Utilities
{
    public static class Utils
    {
        ...
        public static List<ValidationFailure> Transform(List<RuleEngineResult> ruleResult)
        {
            List<ValidationFailure> transformedFailures = new List<ValidationFailure>();
            TransformRecursive(ruleResult, transformedFailures);
            return transformedFailures;
        }

        private static void TransformRecursive(List<RuleEngineResult> ruleResult, List<ValidationFailure> transformedFailures)
        {
            foreach (var result in ruleResult)
            {
                if (!result.IsSuccess)
                {
                    transformedFailures.Add(new ValidationFailure(result.RuleName, result.Outcome));
                }

                if (result.ChildResult != null && result.ChildResult.Any())
                {
                    TransformRecursive(result.ChildResult, transformedFailures);
                }
            }
        }
    }
