...
namespace Application.Common.Behaviours
{
    public class CachingBehaviour<TRequest, TResponse> : IPipelineBehavior<TRequest, TResponse> where TRequest : notnull
    {
        ...
        public CachingBehaviour(IWebHostEnvironment env, IConfiguration configuration, ILogger<CachingBehaviour<TRequest, TResponse>> logger, IMemoryCacheService memoryCacheService = null, IRedisCacheService redisCacheService = null)
        {
            _logger = logger;
        ...
        private async Task HandleCommand(InvalidateCacheAttribute commandRequest)
        {
            foreach (var type in commandRequest.Queries)
            {
                ...
                if (queryType == null) continue; // query is not cached — nothing to invalidate
                ...
                {
                    await _memoryCacheService.RemoveCacheValue(key);
                }
            }
        }
