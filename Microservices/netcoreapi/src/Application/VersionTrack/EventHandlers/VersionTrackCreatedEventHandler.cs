...
{
    /// <summary>
    /// For Creating handler for the domain event notification,created VersionTrackCreatedEventHandler 
    /// class that implements the INotificationHandler interface as shown below.
    /// </summary>
    public class VersionTrackCreatedEventHandler : INotificationHandler<DomainEventNotification<VersionTrackCreatedEvent>>
    {
        private readonly ILogger<VersionTrackCreatedEventHandler> _logger;
        private IConfiguration _configuration;
        private readonly IPublishEventDataAccess _publishEventDataAccess;


        /// <summary>
        /// Instantiation of VersionTrackCreatedEventHandler class
        /// </summary>
        /// <param name="logger"></param>
        /// <param name="configuration"></param>
        public VersionTrackCreatedEventHandler(IConfiguration configuration, ILogger<VersionTrackCreatedEventHandler> logger, IPublishEventDataAccess publishEventDataAccess)
        {
        ...
        public Task Handle(DomainEventNotification<VersionTrackCreatedEvent> notification, CancellationToken cancellationToken)
        {
            var domainEvent = notification.DomainEvent;
            domainEvent.VersionTrackCreatedObject.AuditableSourceEventName = nameof(VersionTrackCreatedEvent);
        ...
        private async void AddEventDataForAuditLog(VersionTrackCreatedEvent domainEvent)
        {
            try
            {
        ...
        private PublishEventData PrepareEventDataForAuditLog(VersionTrackCreatedEvent domainEvent)
        {
            //prepare event data for Audit Log
            var newObject = domainEvent.VersionTrackCreatedObject;
