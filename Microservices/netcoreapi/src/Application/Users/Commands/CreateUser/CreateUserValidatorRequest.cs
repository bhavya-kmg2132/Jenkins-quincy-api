...
{
    public class CreateUserValidatorRequest : AbstractValidator<CreateUserRequest>
    {
        /// <summary>
        /// Validates User create request
        /// </summary>
        public CreateUserValidatorRequest()
        {
            //1) Id
            //It is mandatory.
            //RuleFor(x => x.Id).Cascade(CascadeMode.Stop)
            //   .NotEmpty()
            //   .WithMessage(Resources.ErrorMessages.CreateUserRequestValidator_Id_NotEmpty);

            //2) oid
            //It is mandatory.
            RuleFor(x => x.oid).Cascade(CascadeMode.Stop)
              .NotEmpty()
              .WithMessage(Resources.ErrorMessages.CreateUserRequestValidator_oid_NotEmpty);

            //3) dipslay_name
            //It is mandatory.
            RuleFor(x => x.display_name).Cascade(CascadeMode.Stop)
               .NotEmpty()
               .WithMessage(Resources.ErrorMessages.CreateUserRequestValidator_display_name_NotEmpty);
            ...
            RuleFor(x => x.Email).Cascade(CascadeMode.Stop)
               .NotEmpty()
               .WithMessage(Resources.ErrorMessages.CreateUserRequestValidator_Email_NotEmpty)
